...
def atualizar_posto_view(request):
    if request.method == 'POST':
        nome = request.POST.get('nome')
        endereco = request.POST.get('endereco')
        latitude = request.POST.get('latitude')
...
def adicionar_preco_view(request):
    if request.method == 'POST':
        ...
        return render(request, 'forms_preco.html', context={'postos': Posto.objects.all()})
...
def adicionar_comodidade_view(request):
    if request.method == 'POST':
        ...
        postos = Posto.objects.all()
        return render(request, 'forms_comodidade.html', context={'postos': postos})
